...
public class KhachHangRepository {
    ...
    public ArrayList<KhachHang> getAll(){
        ...
        try (Connection conn = DBContext1.getConnection();
            ...
            while (rs.next()){
                 ...
                 KhachHang khachHang = new KhachHang(maKH, hoTen, gioiTinh, ngaySinh, diaChi, Email, dienThoai);
                 list.add(khachHang);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Boolean addNew(KhachHang kh){
        String sql = "INSERT INTO KhachHang (HoTen, GioiTinh, NgaySinh, DiaChi, Email, DienThoai) VALUES (?,?,?,?,?,?)";
        try (Connection conn = DBContext1.getConnection();
                PreparedStatement pst = conn.prepareStatement(sql)){
    ...
    public Boolean Update(KhachHang kh){
        String sql = "UPDATE KhachHang SET HoTen = ?, GioiTinh = ?, NgaySinh = ?, DiaChi = ?, Email = ?, DienThoai = ? WHERE MaKhachHang = ?";
        try (Connection conn = DBContext1.getConnection();
                PreparedStatement pst = conn.prepareStatement(sql)){
            pst.setObject(1, kh.getHoTen());
            pst.setObject(2, kh.getGioiTinh());
            ...
            pst.setObject(5, kh.getEmail());
            pst.setObject(6, kh.getSoDienThoai());
            pst.setObject(7, kh.getMaKhachHang());
            int kq = pst.executeUpdate();
            return kq > 0;
        } catch (Exception e) {
            e.printStackTrace();
